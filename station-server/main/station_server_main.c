...
void vTaskCastIP(void * pvParameters)
{
    ...
    while (1)
    {
        ...
        broadcastAddr.sin_port = htons(PORT_CASTIP);
...
void vTaskServer(void * pvParameters)
{
    while (1) {
        ...
        dest_addr_ip4->sin_port = htons(PORT_RECV);
        ...
        ESP_LOGI(TAG_SERVER, "Socket bound, port %d", PORT_RECV);
